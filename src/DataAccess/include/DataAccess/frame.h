/*
 * One ping data trans to a frame stored in the queue
 * Organization: PengCheng Lab
 * Author:	qiukang, cabin
 * Date:	Dec 17, 2020
 *--------------------------------------------------------------------*/
#ifndef FRAME_H_
#define FRAME_H_
...
double getTimeStamp(unsigned short year,
                    unsigned char month,
                    unsigned char day,
                    unsigned char hour,
...
class pingFrame: public Frame{
    public:
        pingFrame(xtf_pingpacket* p_pingPacket,xtf_fileheader fileHeader):
            ...
            slantRangePort(p_pingPacket->pingChanHeader[0].SlantRange),
            slantRangeStbd(p_pingPacket->pingChanHeader[1].SlantRange),
            bytesPerSamplePort(fileHeader.chaninfo[p_pingPacket->pingChanHeader[0].ChannelNumber].BytesPerSample),
            bytesPerSampleStbd(fileHeader.chaninfo[p_pingPacket->pingChanHeader[1].ChannelNumber].BytesPerSample),
            numSamplesPort(p_pingPacket->pingChanHeader[0].NumSamples),
            numSamplesStbd(p_pingPacket->pingChanHeader[1].NumSamples){
    ...
    public:
        ...
        float sensorHeading;
        //float slantRange;
        float slantRangePort;
        float slantRangeStbd;
        //unsigned short bytesPerSample;
        unsigned short bytesPerSamplePort;
        unsigned short bytesPerSampleStbd;
        //unsigned int numSamples;
        unsigned int numSamplesPort;
        unsigned int numSamplesStbd;
        void * p_data[2];
};
...
class bathmetryFrame: public Frame{
    public:
        bathmetryFrame(xtf_bathmetrypacket* p_bathmetryPacket,xtf_fileheader fileHeader):
            ...
            bytesPerSample(fileHeader.chaninfo[p_bathmetryPacket->bathymetryChannelNum].BytesPerSample){
